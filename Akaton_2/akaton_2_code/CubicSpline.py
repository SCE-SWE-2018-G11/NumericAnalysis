...
def CubicSplineDerivatives(x_values, y_values, first_derivative, last_derivative):
    ...
    for i in range(1, len(x_values) - 1):
        ...
        intervals[i - 1] / 6, (intervals[i - 1] + intervals[i]) / 3, intervals[i] / 6) + tuple(
            0 for _ in range(len(x_values) - i - 2)) + (
                   (y_values[i + 1] - y_values[i]) / intervals[i] - (y_values[i] - y_values[i - 1]) / intervals[
                       i - 1],),)
        ...
        coefficients = (
            # Formula for S_i taken from presentation slide 11, and ran through WolframAlpha
                ...
                i + 1] * interval_size ** 2) + x_values[i + 1] * (
                     ...
                     i])) / (6 * interval_size),
            (derivatives[i] * (3 * x_values[i + 1] ** 2 - interval_size ** 2) + 6 * (y_values[i + 1] - y_values[i]) +
             derivatives[i + 1] * interval_size ** 2 - 3 * x_values[i] ** 2 * derivatives[i + 1]) / (6 * interval_size),
